...
# --selftest: a scanner that never fires is worse than no scanner. This plants one of each
# thing that must never be committed and asserts the gate catches it. The 'no \b' in the
# password rule is here because without it the scanner missed $ChadminPassword - i.e. the
# exact credential the estate rule names. Caught by this test, not by review.
if '--selftest' in sys.argv:
    import tempfile, subprocess
    d = tempfile.mkdtemp()
    k = 'PMBDC-FXVM3-' + 'T777P-' + 'N4FY8-PKFF4'
    w = 'Sup3r' + 'Secret#Pass'
    h = 'ceb8ad2ebbc037042ab92f05d885c421' + '89ea1101af6690130e74597cec1f168e'
    open(os.path.join(d,'canary.ps1'),'w').write(
        "$SqlKey = '%s'\n$Chadmin" "Password = '%s'\n$Webhook" "Secret = '%s'\n" % (k, w, h))
    r = subprocess.run([sys.executable, os.path.abspath(__file__), d],
                       capture_output=True, text=True)
    out = r.stdout
    need = ['PMBDC', 'ChadminPassword', 'WebhookSecret']
    missed = [n for n in need if n not in out]
    if r.returncode == 0 or missed:
        print('SELFTEST FAILED - the gate did not catch:', missed or '(exit was 0)')
        print(out)
        sys.exit(2)
    print('selftest ok - planted MAK, password and secret were all caught')
    sys.exit(0)
SKIP_DIR={'.git'}
...
W3GGN-FT8W3-Y4M27-J84CP-Q3VJ9 JCKRF-N37P4-C2D82-9YXRT-4M63B KNC87-3J2TX-XB4WP-VCPJV-M4FWM""".split())
...
RULES=[
 ...
 ('assigned password',re.compile(r'(?i)(?:password|passwd|pwd|secret|apikey|api_key|token)\s*=\s*["\']([^"\'\s]{8,})["\']')),
            ...
            for i,line in enumerate(t.splitlines(),1):
                ...
                for cat,rx in RULES:
                    ...
                    if cat=='assigned password':
                        ...
                        if re.search(r'[|()\[\]]', v) and '|' in v: continue   # a regex, not a secret
                        # publicKeyToken is the PUBLIC assembly identity in every Windows
                        # unattend XML - same value on every machine, and named "public".
                        if re.search(r'(?i)publicKeyToken', line): continue
                        # __LIKE_THIS__ is a substitution placeholder, not a value.
                        if re.match(r'^__.*__$', v): continue
                        if '$' in v: continue
                    add(cat,p,i,line)
