...
# yc-payload-version.txt was a FROZEN ARTEFACT from the August golden-image build. It said
# payload_version = v265, "Yallacloud catalog v2.8.0" and host = WIN2016EFI on every guest,
# a month after both had moved on - and yc-compliance, the catalogue banner and yc-event all
# report from it, so every one of them was repeating it. A version file nobody regenerates is
# worse than no version file, because it reads like a fact. Stamped here, every build.
if [ -n "$NEWCAT" ]; then
  {
    echo "payload_version = $NEWCAT"
    echo "catalogue       = $NEWCAT"
    echo "built_utc       = $(date -u +%Y-%m-%dT%H:%M:%SZ)"
    [ -f "$SRC/MANIFEST.json" ] && echo "manifest_sha256 = $(sha256sum "$SRC/MANIFEST.json" | awk '{print $1}')"
    echo "note            = generated by build-payload.sh. Install time is in yc-update.log,"
    echo "                  not here - this file describes the PAYLOAD, not the machine."
  } > "$SRC/yc-payload-version.txt"
  echo "stamped yc-payload-version.txt = $NEWCAT"
fi
